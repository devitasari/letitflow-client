...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    FETCH_MY_ANSWERS (state, answers) {
  ...
  actions: {
    fetchJobs ({ dispatch, commit, state }, payload) {
      axios({
        url: `questions/jobs`,
        method: 'GET'
      })
        .then(({ data }) => {
          commit('FETCH_JOBS', data)
        })
        .catch(err => {
          console.log(err.response)
        })
    },
    deleteTag ({ dispatch, commit, state }, payload) {
    ...
    fetchMyAnswers ({ dispatch, commit, state }) {
      return new Promise((res, rej) => {
        axios({
          url: '/answers/my',
          method: 'GET',
